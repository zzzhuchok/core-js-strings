...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, startIndex) + str.slice(lastIndex);
}
...
function isPalindrome(str) {
  const strChar = str
    ...
    .join('');
  const str2 = strChar.split('').reverse().join('');
  return strChar === str2;
}
...
function findLongestWord(sentence) {
  ...
  arrWord.forEach((el) => {
    if (!objRes[el.length]) objRes[el.length] = [el];
    else objRes[el.length].push(el);
  });
  const resArr = Object.keys(objRes);
  return objRes[resArr[resArr.length - 1]][0];
}
...
function invertCase(str) {
  return str
    ...
    .map((el) => {
      ...
      if (charCode < 65 || charCode > 122) return el;
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((el) => {
      const indexInput = input.indexOf(el);
      return indexInput !== -1 ? output[indexInput] : el;
    })
    .join('');
}
